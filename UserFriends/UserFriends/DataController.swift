...
//  UserFriends
//
//  Created by Dan Lovell on 7/03/22.
//
...
class DataController: ObservableObject {
    ...
    // *******************************************************************
    let container = NSPersistentContainer(name: "UserFriends")
    ...
    init() {
        container.loadPersistentStores {description, error in
            if let error = error {
                print("CoreData failed to load UserFriends data: \(error.localizedDescription)")
            }
        }
    }
